...
// :: Proxies ::
//
// ddk::PbusProtocolProxy is a simple wrappers around platform_bus_protocol_t. It does
// not own the pointers passed to it.
//
// :: Mixins ::
//
// ddk::PbusProtocol is a mixin class that simplifies writing DDK drivers that
...
//       : PbusDeviceType("my-platform-bus", parent) {}
//
...
private:
    ...
    static zx_status_t DeviceEnable(void* ctx, uint32_t vid, uint32_t pid, uint32_t did, bool enable) {
...
public:
    PbusProtocolProxy(platform_bus_protocol_t* proto)

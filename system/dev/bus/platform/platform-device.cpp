...
zx_status_t PlatformDevice::Init(const pbus_dev_t* pdev) {
    auto status = resource_tree_.Init(pdev);
    if (status != ZX_OK) {
        return status;
    }

    size_t device_count = resource_tree_.DeviceCount();
    printf("device_count: %zu\n", device_count);

    fbl::AllocChecker ac;
    device_index_.reserve(device_count, &ac);
    if (!ac.check()) {
        return ZX_ERR_NO_MEMORY;
    }
...
zx_status_t PlatformDevice::MapMmio(uint32_t index, uint32_t cache_policy, void** out_vaddr,
    ...
    DeviceResources* dr = device_index_[0];
...
zx_status_t PlatformDevice::GetDeviceInfo(DeviceResources* dr, pdev_device_info_t* out_info) {
...
zx_status_t PlatformDevice::GetDeviceInfo(pdev_device_info_t* out_info) {
    return GetDeviceInfo(device_index_[0], out_info);
}
...
zx_status_t PlatformDevice::RpcGetMmio(DeviceResources* dr, uint32_t index, zx_paddr_t* out_paddr,
...
zx_status_t PlatformDevice::RpcGetInterrupt(DeviceResources* dr, uint32_t index, uint32_t* out_irq,
...
zx_status_t PlatformDevice::RpcGetBti(DeviceResources* dr, uint32_t index, zx_handle_t* out_handle,
                                      uint32_t* out_handle_count) {
...
zx_status_t PlatformDevice::RpcGpioConfig(DeviceResources* dr, uint32_t index, uint32_t flags) {
...
zx_status_t PlatformDevice::RpcGpioSetAltFunction(DeviceResources* dr, uint32_t index,
                                                  uint64_t function) {
    if (bus_->gpio() == nullptr) {
        return ZX_ERR_NOT_SUPPORTED;
    }
...
zx_status_t PlatformDevice::RpcGpioRead(DeviceResources* dr, uint32_t index, uint8_t* out_value) {
    if (bus_->gpio() == nullptr) {
        return ZX_ERR_NOT_SUPPORTED;
    }
    if (index >= dr->gpio_count()) {
...
zx_status_t PlatformDevice::RpcGpioWrite(DeviceResources* dr, uint32_t index, uint8_t value) {
    if (bus_->gpio() == nullptr) {
        return ZX_ERR_NOT_SUPPORTED;
    }
    if (index >= dr->gpio_count()) {
...
zx_status_t PlatformDevice::RpcGpioGetInterrupt(DeviceResources* dr, uint32_t index, uint32_t flags,
                                                zx_handle_t* out_handle,
                                                uint32_t* out_handle_count) {
    if (bus_->gpio() == nullptr) {
        return ZX_ERR_NOT_SUPPORTED;
...
zx_status_t PlatformDevice::RpcGpioReleaseInterrupt(DeviceResources* dr, uint32_t index) {
    if (bus_->gpio() == nullptr) {
        return ZX_ERR_NOT_SUPPORTED;
    }
    if (index >= dr->gpio_count()) {
...
zx_status_t PlatformDevice::RpcGpioSetPolarity(DeviceResources* dr, uint32_t index,
                                               uint32_t flags) {
    if (bus_->gpio() == nullptr) {
        return ZX_ERR_NOT_SUPPORTED;
    }
...
zx_status_t PlatformDevice::RpcCanvasConfig(DeviceResources* dr, zx_handle_t vmo, size_t offset,
...
zx_status_t PlatformDevice::RpcCanvasFree(DeviceResources* dr, uint8_t canvas_idx) {
    if (bus_->canvas() == nullptr) {
        return ZX_ERR_NOT_SUPPORTED;
    }
...
zx_status_t PlatformDevice::RpcScpiGetSensor(DeviceResources* dr, char* name, uint32_t *sensor_id) {
...
zx_status_t PlatformDevice::RpcScpiGetSensorValue(DeviceResources* dr, uint32_t sensor_id,
...
zx_status_t PlatformDevice::RpcScpiGetDvfsInfo(DeviceResources* dr, uint8_t power_domain,
                                               scpi_opp_t* opps) {
    if (bus_->scpi() == nullptr) {
        return ZX_ERR_NOT_SUPPORTED;
    }
...
zx_status_t PlatformDevice::RpcScpiGetDvfsIdx(DeviceResources* dr, uint8_t power_domain,
                                              uint16_t* idx) {
    if (bus_->scpi() == nullptr) {
        return ZX_ERR_NOT_SUPPORTED;
    }
...
zx_status_t PlatformDevice::RpcScpiSetDvfsIdx(DeviceResources* dr, uint8_t power_domain,
                                              uint16_t idx) {
    if (bus_->scpi() == nullptr) {
        return ZX_ERR_NOT_SUPPORTED;
    }
...
zx_status_t PlatformDevice::RpcI2cTransact(DeviceResources* dr, uint32_t txid, rpc_i2c_req_t* req,
...
zx_status_t PlatformDevice::RpcI2cGetMaxTransferSize(DeviceResources* dr, uint32_t index,
                                                     size_t* out_size) {
    if (bus_->i2c_impl() == nullptr) {
        return ZX_ERR_NOT_SUPPORTED;
    }
...
zx_status_t PlatformDevice::RpcClkEnable(DeviceResources* dr, uint32_t index) {
    if (bus_->clk() == nullptr) {
        return ZX_ERR_NOT_SUPPORTED;
    }
...
zx_status_t PlatformDevice::RpcClkDisable(DeviceResources* dr, uint32_t index) {
    if (bus_->clk() == nullptr) {
        return ZX_ERR_NOT_SUPPORTED;
    }
    if (index >= dr->clk_count()) {
...
zx_status_t PlatformDevice::DdkRxrpc(zx_handle_t channel) {
    ...
    uint32_t device_id = req_header->device_id;
    if (device_id >= device_index_.size()) {
        return ZX_ERR_OUT_OF_RANGE;
    }
    DeviceResources* dr = device_index_[device_id];
